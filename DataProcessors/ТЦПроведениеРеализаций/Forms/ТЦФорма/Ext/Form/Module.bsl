...
Функция ТЦПолучитьСписокДокументовДляПроведения()
	...
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 100
	|	_ДемоРеализацияТоваров.Ссылка
	|ИЗ
	|	Документ._ДемоРеализацияТоваров КАК _ДемоРеализацияТоваров
	|ГДЕ
	|	_ДемоРеализацияТоваров.Проведен
	|	И НЕ _ДемоРеализацияТоваров.ПометкаУдаления";
...
Процедура ТЦПровестиДокументНаСервере(ДокументСсылка)
	...
	ОценкаПроизводительностиКлиентСервер.ЗакончитьЗамерВремени("Проведение реализации", ВремяНачала);
