...
Функция ТЦПолучитьСписокДокументовДляПроведения()
	...
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 100
	|	_ДемоПоступлениеТоваров.Ссылка
	|ИЗ
	|	Документ._ДемоПоступлениеТоваров КАК _ДемоПоступлениеТоваров
	|ГДЕ
	|	_ДемоПоступлениеТоваров.Проведен
	|	И НЕ _ДемоПоступлениеТоваров.ПометкаУдаления";
...
Процедура ТЦПровестиДокументНаСервере(ДокументСсылка)
	...
	ОценкаПроизводительностиКлиентСервер.ЗакончитьЗамерВремени("Проведение поступления", ВремяНачала);
