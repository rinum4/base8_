...
Функция ТЦИнициализировать() Экспорт
	...
	#Иначе
		...
		ТЦЭлементыДляИзменения = ТЦПолучитьСписокЭлементовДляИзменения();
...
&НаСервере
Функция ТЦПолучитьСписокЭлементовДляИзменения()
	...
	|ГДЕ
	|	НЕ _ДемоКонтрагенты.ПометкаУдаления";
...
Функция ТЦВыполнитьИтерациюТеста()
	...
	Если (ТекущаяДата() - ТЦВремяНачалаТеста) < ТЦОбъект.ДлительностьТеста*60 Тогда
		...
		ИндексЭлемента = ГСЧ.СлучайноеЧисло(0, ТЦЭлементыДляИзменения.ВГраница());
		
		ЭлементСсылка = ТЦЭлементыДляИзменения[ИндексЭлемента];
		ТЦИзменитьЭлементНаСервере(ЭлементСсылка);
		Сообщить("Изменен элемент справочника: " + ЭлементСсылка);
...
Процедура ТЦИзменитьЭлементНаСервере(ЭлементСсылка)
	...
	ОценкаПроизводительностиКлиентСервер.ЗакончитьЗамерВремени("Изменение контрагента", ВремяНачала);
