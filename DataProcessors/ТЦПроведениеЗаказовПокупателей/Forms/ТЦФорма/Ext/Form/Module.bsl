...
Функция ТЦПолучитьСписокДокументовДляПроведения()
	...
	|ГДЕ
	|	_ДемоЗаказПокупателя.Проведен
	|	И НЕ _ДемоЗаказПокупателя.ПометкаУдаления";
...
Процедура ТЦПровестиДокументНаСервере(ДокументСсылка)
	...
	ОценкаПроизводительностиКлиентСервер.ЗакончитьЗамерВремени("Проведение заказов покупателя", ВремяНачала);
