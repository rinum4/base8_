...
Функция ТЦПолучитьСписокДокументовДляПроведения()
	...
	|ГДЕ
	|	_ДемоПеремещениеТоваров.Проведен
	|	И НЕ _ДемоПеремещениеТоваров.ПометкаУдаления";
...
Процедура ТЦПровестиДокументНаСервере(ДокументСсылка)
	...
	ОценкаПроизводительностиКлиентСервер.ЗакончитьЗамерВремени("Проведение перемещения", ВремяНачала);
