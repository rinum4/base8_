...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "2.3.2.195";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("dfc6a0fa-7c7b-4096-9d04-2c67d5eb17a4");
	Обработчик.Процедура = "УправлениеКонтактнойИнформациейСлужебный.ОбновитьСтраныМираПоОКСМ";
...
Процедура ОбновитьСтраныМираПоОКСМ(Параметры, РазмерПорции = 1000) Экспорт
	
	ОбновляемыеСтраныМира = Неопределено;
	Параметры.Свойство("ОбновляемыеСтраныМира", ОбновляемыеСтраныМира);
	
	Если Параметры.ПрогрессВыполнения.ВсегоОбъектов = 0 Тогда
		ОбновляемыеСтраныМира                      = СписокСтранКОбработке();
		Параметры.ПрогрессВыполнения.ВсегоОбъектов = ОбновляемыеСтраныМира.Количество();
	КонецЕсли;
	
	Если ОбновляемыеСтраныМира = Неопределено ИЛИ ОбновляемыеСтраныМира.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Позиция = ОбновляемыеСтраныМира.Количество() - 1;
	КоличествоОбработанных = 0;
	
	Пока Позиция >= 0 И КоличествоОбработанных < РазмерПорции Цикл
		
		ДанныеОКСМ = УправлениеКонтактнойИнформацией.ДанныеКлассификатораСтранМираПоКоду(ОбновляемыеСтраныМира.Получить(Позиция).Код);
		
		Если ДанныеОКСМ <> Неопределено Тогда
			СтранаМира = ОбновляемыеСтраныМира.Получить(Позиция).ПолучитьОбъект();
			ЗаполнитьЗначенияСвойств(СтранаМира, ДанныеОКСМ);
			ОбновлениеИнформационнойБазы.ЗаписатьДанные(СтранаМира);
		КонецЕсли;
		
		ОбновляемыеСтраныМира.Удалить(Позиция);
		Позиция = Позиция - 1;
		КоличествоОбработанных = КоличествоОбработанных + 1;
		
	КонецЦикла;
	
	Если ОбновляемыеСтраныМира.Количество() > 0 Тогда
		Параметры.ОбработкаЗавершена = Ложь;
	КонецЕсли;
	
	Параметры.ПрогрессВыполнения.ОбработаноОбъектов = Параметры.ПрогрессВыполнения.ОбработаноОбъектов + КоличествоОбработанных;
	Параметры.Вставить("ОбновляемыеСтраныМира", ОбновляемыеСтраныМира);
